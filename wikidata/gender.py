...
def log(title, item, text=''):
    pywikibot.output(u'\03{{lightyellow}}item {qid}{text}'.format(qid=item.getID(), text=text))
...
for line in lines:
    if multimode:
        ...
        gender = genders['male']
    if not item.exists():
        pywikibot.output(u'\03{{lightred}}item {qid} does not exist'.format(qid=item.getID()))
        continue
    item.get(force=True)
    if 'itwiki' in item.sitelinks:
        pywikibot.output(u'\03{{lightyellow}}trying to import {propid} for {qid} from itwiki sitelink: {ittitle}'.format(propid=prop,
                                                                                                                         qid=item.getID(),
                                                                                                                         ittitle=item.sitelinks['itwiki']))
    ...
    if not prop in item.claims:
        ...
        pywikibot.output(u'\03{{lightgreen}}{qid}: source "personal name" successfully added'.format(qid=item.getID()))
        continue
    item.get(force=True)
    if prop in item.claims:
        if len(item.claims[prop]) == 1 and item.claims[prop][0].getTarget().getID() == gender.getID():
            ...
            try:
                ...
                pywikibot.output(u'\03{{lightgreen}}{qid}: removed duplicate claim for {propid}'.format(qid=item.getID(), propid=prop))
                item.claims[prop][0].addSource(reference, bot=1)
                pywikibot.output(u'\03{{lightgreen}}{qid}: source "personal name" successfully added'.format(qid=item.getID()))
            except:
                pass
