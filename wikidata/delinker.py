	...
	for template in code.ifilter_templates():
		if unicode(template.name).replace(basepage.title(),'').lower().strip()=='/row':
			...
			if template.has_param('prop'):
				prop_only=unicode(template.get('prop').value).lower().replace('p','').split('-')
			...
			summary=u'[[User:SamoaBot/Delinker|SamoaBot Delinker]]: migrating [[{itemfrom}]] to [[{itemto}]] as ordered by [[User:{by}|{by}]] {reason}'.format(itemfrom=itemfrom.getID().upper(),itemto=itemto.getID().upper(),by=unicode(template.get('by').value),reason=unicode(template.get('reason').value))
			count=0
			...
			if len(refs)==0:
				...
				for page in refs:
					...
					for prop in item.claims:
						if prop_only and (not prop.lower().replace('p','') in prop_only):
							continue
