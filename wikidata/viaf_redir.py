...
viols=pywikibot.Page(site,'Database reports/Constraint violations/'+viaf.upper(),ns=4).get(force=True)
for match in re.finditer(ur'^\*\s*\[\[(?P<item>[Qq]\d+)\]\]\:\s*\['+regex+'\s+\d+\](\,\s*\['+regex+'\s+\d+\])+(\n|$)',viols,flags=re.MULTILINE):
	...
	if viaf in item.claims and len(item.claims[viaf])>1:
		...
		for claim in item.claims[viaf]:
			...
			got=re.match(regex+'$',urllib2.urlopen(url).geturl()).group(1)
			if got!=prev:
		...
		if len(removable)>0:
			...
			item.removeClaims(removable,summary='removed redirected VIAF id{s}'.format(s=s))
